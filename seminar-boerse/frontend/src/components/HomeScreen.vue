...
const notificationsRaw = ref<any[]>([]);

onMounted(async () => {
  notificationsRaw.value = await fetchNotifications();
  console.log("Loaded notifications:", notificationsRaw.value);
});

const notifications = computed(() =>
  notificationsRaw.value.map((n) => ({
    id: n.id,
    message: handleNotification(n.type, n.payload),
  }))
);
